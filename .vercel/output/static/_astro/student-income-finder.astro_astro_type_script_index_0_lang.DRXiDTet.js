const d=window.QUIZ_DATA;let a=0,o={};const g=document.getElementById("quiz-intro"),v=document.getElementById("quiz-questions"),$=document.getElementById("quiz-loading"),y=document.getElementById("quiz-results"),B=document.getElementById("progress-bar"),I=document.getElementById("progress-text"),p=document.getElementById("question-container"),E=document.getElementById("prev-btn"),k=document.getElementById("next-btn"),L=document.getElementById("start-quiz");L.addEventListener("click",()=>{f("questions"),h()});E.addEventListener("click",()=>{a>0&&(a--,h())});k.addEventListener("click",()=>{const e=d.questions[a],t=S(e);if(!t||Array.isArray(t)&&t.length===0){alert("Please select an option to continue");return}o[e.id]=t,a<d.questions.length-1?(a++,h()):R()});function h(){const e=d.questions[a],t=d.questions.length,s=(a+1)/t*100;B.style.width=`${s}%`,I.textContent=`Question ${a+1} of ${t}`,E.disabled=a===0,k.textContent=a===t-1?"See Results →":"Next →";const n=o[e.id];p.innerHTML=`
          <div class="question-card">
            <h2 class="question-text">${e.question}</h2>
            <div class="options-grid ${e.type==="multiple"?"multiple":""}">
              ${e.options.map(i=>`
                <label class="option-card ${q(n,i.value)?"selected":""}">
                  <input 
